...
c(0.0156, 0.642)
```
For the asymptomatic and symptomatic groups the 95% confidence interval of the Kaplan Meier estimates of 6 year survival (taken as 312 week survival) is [**0.309**, **0.690**] and [**0.0156**, **0.642**] respectively. 
...
#### Part c:
Using the same model selection criteria of AIC using the Cox proportional hazards model starting with the hierarchical model with the three way interaction between age, weight and length (and thus all lower order terms) my final model is the null model with a AIC of **128.00** compared to the initial models AIC of **135.44**. Thus, using my final model none of the covariates age, weight and length affect the survival of the ducks and when fitting a model with all 3 covariates, none are significant at $\alpha=0.05$. Therefore using this model the survival of the ducks does not depend on their age, weight or length using the provided data.
```{r, include=F}
#Fitting cox models
...
#### Part d:
Plotting the estimated survivor function for a one year old duck with a weight of 1000g and a length of 250cm using my preferred models for part a and c gives the corresponding plot below. I note that this is the same survivor function as that for any other duck with a different weight, length or age (within the values of the dataset used to produce my model, as may not be able to extrapolate) as both models don't include the covariates age, weight or length.
```{r,echo=F,fig.align='center',fig.width=10,fig.height=5}
...
Calculating the crude central mortality rates $m_{x}$ for male and female pensioners using $\hat{m_{x}}=\frac{D_{x}}{E_{x}^{C}}$ ($D_{x}$ the number of deaths at age $x$ and $E_{x}^{C}$ the central exposed risk at age $x$) and plotting it on the log scale gives the below graph. In it we can see that $log(m_{x})$ is higher for males than for females at all ages with the exception of ages 63, 95, 96, 102. Thus, overall this graph suggests that $log(m_{x})$ and thus $m_{x}$, the crude central mortality rate is higher for male pensioners than female pensioners.
...
#### Part b:
Calculating $q_{x}$ for males and female using both the assumption of constant force of mortality and uniform distribution of deaths within each year of age gives the corresponding life table (using $l_{60}=100000$).
