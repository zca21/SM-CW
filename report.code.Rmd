...
```
#### Part b:
To check if a Weibull model may be reasonable I plotted $log(\hat{H}(t))$ against $log(t)$ and checked that $log(\hat{H}(t))$ is linear with $log(t)$ with gradient equal to $\alpha$ the shape parameter of the estimated Weibull model. In the below graph the red line has intercept -5.5 and gradient 1.21 (the estimated shape parameter is $\hat{\alpha}=1.211$ in my chosen final Weibull model from part a). The line is a good approximation of the points thus a Weibull distribution is a reasonable model.
